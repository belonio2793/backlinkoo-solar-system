// format-post.ts
// Canonical post formatter — hardened, idempotent, and resilient to AI-generated garbage.
...
// decode entities repeatedly to handle double-encoded values like &amp;quot;
export function decodeEntitiesDeep(s: string, maxDepth = 5): string {
  let prev = String(s || '');
...
export function sanitize(html: string): string {
  ...
  // Unescape safe encoded tags (e.g. &lt;strong&gt;) so they can be normalized
  ...
  out = out.replace(/(href|src)\s*=\s*(["']?)\s*javascript:[^"'>\s]*\1/gi, '');
...
function normalizeLinks(html: string): string {
  ...
  return String(html || '').replace(/<a\b([^>]*)>/gi, (m, rawAttrs) => {
    ...
    const hrefRaw0 = hrefMatch ? (hrefMatch[1] || hrefMatch[2] || hrefMatch[3] || '') : '';
    // decode entities deeply and strip stray encoded quotes or trailing garbage like &quot; or &amp;quot;
    let hrefRaw = decodeEntitiesDeep(String(hrefRaw0 || '')).trim();
    hrefRaw = hrefRaw.replace(/(^["']+|["']+$)/g, '');
    hrefRaw = hrefRaw.replace(/(&quot;|&apos;|&amp;quot;)+$/i, '');
    hrefRaw = hrefRaw.replace(/\s+$/,'');
    const href = normalizeUrl(hrefRaw);

    // If no href or a hash-only or mailto/ftp etc -> keep anchor but do not force target
    if (!href || /^#/.test(href) || /^[a-z0-9.+-]+:/i.test(href) && !/^https?:/i.test(href)) {
    ...
    // Remove existing rel if contains nofollow/ugc but otherwise preserve other rel tokens
...
function promoteShortParagraphsToHeadings(html: string): string {
  return String(html || '').replace(/<p([^>]*)>([\s\S]*?)<\/p>/gi, (m, attrs, inner) => {
    ...
    if (!text) return m;
    // don't promote long paragraphs
    if (text.length < 8 || text.length > 120) return m;
    const words = text.split(/\s+/);
    if (words.length > 16) return m;
...
export function extractTitleFromContent(raw: string): string {
  ...
  // Remove image credits/metadata before extracting
  const cleanedSource = stripImageMetaAndCredits(source).replace(/Photo by[\s\S]{0,200}?Pexels/gi, '');
  // prefer explicit h1
  const h1 = cleanedSource.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
  if (h1) return stripTags(h1[1]).replace(/\*+/g,'').trim();

  // "**Title: ...**"
  const t1 = cleanedSource.match(/\*\*\s*Title\s*:\s*([\s\S]*?)\*\*/i);
  if (t1) return String(t1[1]).replace(/\*+/g,'').trim();

  // meta-like candidate filtering (also exclude 'pexels')
  const forbidden = /(\bfigure\b|\bclass\b|\bitemscope\b|\bitemtype\b|schema\.org|\bimageobject\b|\bcontenturl\b|\bwidth\b|\bheight\b|\bauthor\b|\bcaption\b|\bfeatured\b|\bimg\b|\bmeta\b|\bhref\b|\btarget\b|\brel\b|\bnoopener\b|\bnoreferrer\b|\bitemprop\b|\baria-\w+\b|\brole\b|=|\bpexels\b)/i;

  const sentences = cleanedSource.split(/[.!?]\s+/).map(s => s.trim()).filter(Boolean);
  // prefer first <h2> or first sizable sentence with words>=3
  const h2 = cleanedSource.match(/<h2[^>]*>([\s\S]*?)<\/h2>/i);
  if (h2) return stripTags(h2[1]).replace(/\*+/g,'').trim();

  for (const s of sentences) {
    const cleaned = stripTags(s).replace(/\s{2,}/g, ' ').trim();
    if (cleaned.length > 15 && cleaned.split(/\s+/).length >= 3 && !forbidden.test(cleaned)) {
      return String(cleaned).replace(/\*+/g,'').slice(0, 160).trim();
    }
  }

  // fallback: find best candidate by splitting on newlines/dots and filtering
  const candidates = cleanedSource.split(/\n|\.|!|\?/).map(s => stripTags(s).replace(/\bhttps?:\/\/[\S]+/gi, '').replace(/\s{2,}/g, ' ').trim()).filter(s => s.length > 10 && /[a-z]/i.test(s) && s.split(/\s+/).length >= 3 && !forbidden.test(s));
  const first = candidates[0] || '';
  return String(first).replace(/\*+/g,'').slice(0, 160).trim();
}
