// Background version of automation-post to avoid 504 timeouts by running asynchronously
// This file reuses the same logic from automation-post.js but as a Netlify Background Function
...
exports.handler = async (event) => {
  // Netlify background functions automatically return 202 to the caller while this runs to completion.
  try {
    ...
    if (campaignErr) {
      const { data: legacyRow } = await sb.from('automation').select('*').eq('id', campaignId).maybeSingle();
    ...
    const { data: domainsRaw } = await sb
      .from('domains')
        ...
        if (post && post.url) {
          try {
            ...
            await sb.from('automation_published_links').insert({
              ...
              platform: String(d.domain || 'domain'),
    ...
    try { console.error('automation-post-background error:', err && (err.message || err)); } catch {}
  }
};
